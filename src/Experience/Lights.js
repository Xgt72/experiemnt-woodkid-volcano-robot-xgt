...
export default class Lights {
    ...
    setPointLight() {
        ...
        debugFolder
            .addInput(this.pointLight, "color", { view: "color" })
            .on("change", () => {
                this.pointLight.instance.color.set(this.pointLight.color);
            });

        debugFolder.addInput(this.pointLight.instance, "intensity", {
            min: 0,
            max: 100,
        });

        debugFolder.addInput(this.pointLight.instance, "decay", {
            min: 0,
            max: 10,
        });

        debugFolder.addInput(this.pointLight.instance.position, "z", {
            min: 0,
            max: 10,
        });
    }
